...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{tri_pdf.png}
\caption{The PDF of $T$}
\label{fig:tri_pdf}
\end{figure}
\item Find the theoretical expressions for the PDF and CDF of T. \\
\solution Given $T = U_1 + U_2$ \\
Let $x$ : Represent values of $U_1$ \\
and $y$ : Represent values of $U_2$ \\
We Know that $x,y \in (0,1)$ $\Rightarrow 0 < t < 2 $\\
For $0 < t \le 1$
\begin{align}
    F_T(t) &= \int_{y=0}^{t} \int_{x=0}^{t-y} 1 dx dy \\
    F_T(t) &= \int_{y=0}^{t} (t-y) dy \\
    F_T(t) &= \brak{ty - \frac{y^2}{2}} \Biggr|_{0}^{z} \\
    F_T(t) &= \frac{t^2}{2}
\end{align}
For $ 1< t \le 2$
\begin{align}
    F_T(t) &= 1 - \int_{y=t-1}^{1} \int_{x=t-y}^{1} 1 dx dy \\
    F_T(t) &= 1 - \int_{y=t-1}^{1} (1-t+y) dy \\
    F_T(t) &= 1 - \brak{y-ty+\frac{y^2}{2}}\Biggr|_{t-1}^{1} \\
    F_T(t) &= 1 - \brak{1-t+\frac{1}{2}} + \brak{t-1-t(t-1)+\frac{(t-1)^2}{2}} \\
    F_T(t) &= 1-1+t-\frac{1}{2}+t-1-t^2+t+\frac{t^2}{2} + \frac{1}{2} -t \\
    F_T(t) &= - \frac{t^2}{2} +2t -1
\end{align}
For $t < 0$ $F_T(t) = 0 $ \\
For $t > 2 $ $F_T(t) = F_T(2) = 1$
\begin{align}
    \therefore F_T(t) &= 
    \begin{cases}
    0 & t < 0 \\
    \frac{t^2}{2} & 0 \le t \le 1 \\
    - \frac{t^2}{2} +2t -1 & 1 < t \le 2 \\
    1 & t > 2
    \end{cases}
\end{align}
We know that
...
\solution From the figures Fig:\ref{fig:tri_cdf} and Fig:\ref{fig:tri_pdf} we can verify the theoretical and experimental plots.
\end{enumerate}
\end{document}
